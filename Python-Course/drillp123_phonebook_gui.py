...
def load_gui(self):
	...
	self.txt_sourceDir = tk.Entry(self.master,text='Source directory goes here...',width=65)
	self.txt_sourceDir.grid(row=1,column=0,rowspan=1,columnspan=6,padx=(10,10),pady=(10,0),sticky=NSEW)
	self.txt_destDir = tk.Entry(self.master,text='Destination directory goes here...',width=65)
	...
	self.btn_addText = tk.Button(self.master,width=18,height=2,text='Add Text files',command=lambda: drillp123_phonebook_func.onSelect(self))
	self.btn_addText.grid(row=5,column=4,padx=(10,0),pady=(15,10),sticky=W)
	
	drillp123_phonebook_func.create_db(self)
	drillp123_phonebook_func.onRefresh(self)
